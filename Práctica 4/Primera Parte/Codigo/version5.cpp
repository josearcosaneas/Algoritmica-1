...
void backtracking (int primerComensal, int n_comensales, int nivel, int comensal, int &cota_local, int &cota_global, int solucion[], vector<bool> &comensales_sentados, int **matriz) {
	comensales_sentados[comensal] = true;
	solucion[nivel - 1] = comensal;
	int siguiente_comensal = primerComensal;
	int iteraciones = 0;

	while(iteraciones < n_comensales){
		if (comensales_sentados[siguiente_comensal] == false) {			
			cota_local = cota_local + matriz[comensal][siguiente_comensal];
			if (cota_local > cota_global) {				
				backtracking(primerComensal, n_comensales, nivel + 1, siguiente_comensal, cota_local, cota_global, solucion, comensales_sentados, matriz);
			}
		...
		iteraciones++;
		siguiente_comensal++;
		if(siguiente_comensal == n_comensales)
			siguiente_comensal = 0;
	}
}
...
int main(int argc, char const *argv[]){
	...
	int cota_local_max = 0, cota_global_max = 0;
	...
	for(int i = 0; i < n; i++){
		backtracking(i, n, 1, 0, cota_local, cota_global, solucion, comensales_sentados, matriz);
		if(cota_local >= cota_local_max){
			cota_local_max = cota_local;
			cota_global_max = cota_global;
			for(int j = 0; j < n; j++)
				solucion_final[j] = solucion[j];
		}
		cota_local = 0;
		cota_global = 0;
		for(int j = 0; j < n; j++){
			comensales_sentados[j] = false;
			solucion[j] = 0;
		}
	}
	...
	cout << endl << "Cota local = " << cota_local_max << endl;
	cout << endl << "Cota global = " << cota_global_max << endl;
